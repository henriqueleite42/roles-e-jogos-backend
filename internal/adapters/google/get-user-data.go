...
func (self *googleAdapter) GetUserData(accessToken string) (*adapters.GetAuthenticatedUserDataOutput, error) {
	...
	if err != nil {
		self.logger.Error().Err(err).Msg("fail to build request to get discord user data")
	...
	self.logger.Trace().Msg("do request to get discord user data")
	userDataRes, err := self.httpClient.Do(req)
	if err != nil {
		self.logger.Error().Err(err).Msg("fail to make request to get discord user data")
		return nil, errors.New("fail to make request")
	}
	defer userDataRes.Body.Close()
	self.logger.Trace().Msg("request to get discord user data done")
